...
**Precautions**. By default, *TESS-cont* uses the *Gaia* DR3 catalog. However, the TIC catalog (and SPOC PDCSAP) is stacked to *Gaia* DR2. Therefore, to understand the TESS PDCSAP photometry, the DR2 catalog should be selected as an [OPTIONAL](#optional--optional-parameters) parameter: ```gaia_catalog: DR2```. **We highly encourage to run *TESS-cont* based on the DR2 AND DR3 catalogs to ensure that there are no major differences**. If there were, it would be recomendable to **use the DR3 contamination metrics** to correct the PDCSAP/SAP photometry from crowding as explained [here](https://heasarc.gsfc.nasa.gov/docs/tess/UnderstandingCrowding.html). 
